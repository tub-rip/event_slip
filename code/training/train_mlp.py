...
python3 train_mlp.py \
 ...
 --checkpoint_dir /home/thilo/workspace/data/vt_snn/models/final/mlp/ \
 --optimizer RMSprop \
 --repetition 01
...
class TrainMLP:
    ...
    def __init__(self, data_dir, sample_file, checkpoint_dir, batch_size, hidden_size, lr, optimizer_name, epochs, repetition) -> None:
        ...
        self.result_name = f"{set_name[len('pre_'):]}_mlp_opti{optimizer_name}_batch{batch_size}_lr{lr}_maxEpoch{self.epochs}_{self.repetition}"
        self.writer = SummaryWriter(f"/home/thilo/workspace/data/tensorboard/final/models/{self.result_name}")    
        #self.writer = SummaryWriter(f"/home/thilo/workspace/data/tensorboard/final/{set_name[len('pre_'):]}_mlp_opti{optimizer_name}_batch{batch_size}_lr{lr}_maxEpoch{self.epochs}_{self.repetition}")    

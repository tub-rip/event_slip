...
def generate_rand_values(isaac_settings, size_test, size_train, texture_keys=False, mass_keys=False, pick_grip_translation_keys=False, sphere_light_position_keys=False, gripping_depth_keys=False, width_scale_keys=False, height_scale_keys=False, dome_light_intensity_keys=False, sphere_light_intensity_keys=False):
    ...
    if sphere_light_position_keys:
        ...
        sphere_light_position_keys_train = []   
        for i in range(size_test):
            sphere_light_position_keys_test.append(get_sphere_light_position_value(isaac_settings))
        while len(sphere_light_position_keys_train) < size_train:
            key = get_sphere_light_position_value(isaac_settings)
            if not key in sphere_light_position_keys_test:
                sphere_light_position_keys_train.append(key)
        test_collection["sphere_light_position_keys"] = sphere_light_position_keys_test
        train_collection["sphere_light_position_keys"] = sphere_light_position_keys_train
    
    if sphere_light_intensity_keys:
        sphere_light_intensity_keys_test = []
        sphere_light_intensity_keys_train = []
        sphere_light_intensity_range = isaac_settings["environment_config"]["sphere_light"]["intensity_range"]
        for i in range(size_test):
            sphere_light_intensity_keys_test.append(random.uniform(sphere_light_intensity_range[0], sphere_light_intensity_range[1]))
        while len(sphere_light_intensity_keys_train) < size_train:
            key = random.uniform(sphere_light_intensity_range[0], sphere_light_intensity_range[1])
            if not key in sphere_light_intensity_keys_test:
                sphere_light_intensity_keys_train.append(key)
        test_collection["sphere_light_intensity_keys"] = sphere_light_intensity_keys_test
        train_collection["sphere_light_intensity_keys"] = sphere_light_intensity_keys_train
        
    if dome_light_intensity_keys:
        dome_light_intensity_keys_test = []
        dome_light_intensity_keys_train = []
        dome_light_intensity_range = isaac_settings["environment_config"]["dome_light"]["intensity_range"]
        # print('dome_light_intensity_range', dome_light_intensity_range)
        # print('random dome', random.uniform(dome_light_intensity_range[0], dome_light_intensity_range[1]))
        for i in range(size_test):
            dome_light_intensity_keys_test.append(random.uniform(dome_light_intensity_range[0], dome_light_intensity_range[1]))
        while len(dome_light_intensity_keys_train) < size_train:
            key = random.uniform(dome_light_intensity_range[0], dome_light_intensity_range[1])
            if not key in dome_light_intensity_keys_test:
                dome_light_intensity_keys_train.append(key)
        test_collection["dome_light_intensity_keys"] = dome_light_intensity_keys_test
        train_collection["dome_light_intensity_keys"] = dome_light_intensity_keys_train
        

    return test_collection, train_collection
...
def generate_settings(settings, test_size, train_size):
    ...
    for k in range(test_size): # special setup, visualization
        settings_copy = copy.deepcopy(settings)
        i = 1 # special setup, visualization
        if texture_keys:
        ...
        if mass_keys:
            mass = 0.01 if k == 0 else 0.085
            settings_copy["isaac_sim"]["cube_config"]["mass"] = mass # special setup, visualization
...
if __name__ == "__main__":
    ...
    base_output_folder_test = f'/home/thilo/workspace/data/evaluation_sets/data020_test_all_same_but_slip_tiny{test_size}_v2e_thres15'
    base_output_folder_train = f'/home/thilo/workspace/data/evaluation_sets/data020_train_all_same_but_slip_tiny{test_size}_v2e_thres15'
